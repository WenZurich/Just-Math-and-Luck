#!/usr/bin/env node
/**
 * Listen pipeline for whynottv (isolated from gooaye).
 * Optional: clip public enclosure audio + faster-whisper STT.
 * Always: merge scripts/whynottv-stock-analysis.json → public/data/whynottv-episodes.json
 *
 *   node scripts/listen-whynottv-episodes.mjs --merge-only
 *   node scripts/listen-whynottv-episodes.mjs --eps 153,150 --clip-sec 900
 */
...
const PACK = path.join(ROOT, "scripts/whynottv-stock-analysis.json");
const CACHE = path.join(ROOT, "scripts/cache");
const WAV_DIR = path.join(CACHE, "whynottv-wav");
const TX_DIR = path.join(CACHE, "whynottv-transcripts");
...
function mergePack(catalog, pack) {
  ...
  catalog.listenPipeline = {
    script: "scripts/listen-whynottv-episodes.mjs",
    analysisPack: "scripts/whynottv-stock-analysis.json",
