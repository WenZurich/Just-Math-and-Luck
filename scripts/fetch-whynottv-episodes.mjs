#!/usr/bin/env node
/**
 * Fetch WhynotTV Podcast from official Anchor RSS (iTunes id 1824936911).
 * Writes public/data + docs/data. Preserves listened enrichment.
 */
...
const FEED = "https://anchor.fm/s/106f6c76c/podcast/rss";
const APPLE_ID = "1824936911";
const APPLE = "https://podcasts.apple.com/tw/podcast/whynottv-podcast/id1824936911";
const SITE = "https://tairanhe.com";
const OUT_PUBLIC = path.join(ROOT, "public/data/whynottv-episodes.json");
const OUT_DOCS = path.join(ROOT, "docs/data/whynottv-episodes.json");
const UA = "Mozilla/5.0 (compatible; JustMathAndLuck/1.0)";

function curlFeed(url) {
  const r = spawnSync("curl", ["-fsSL", "-A", UA, url], { encoding: "buffer", maxBuffer: 10 * 1024 * 1024 });
...
function pubTw(pub) { const d = new Date(pub); if (Number.isNaN(d.getTime())) return null; return new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Taipei", year: "numeric", month: "2-digit", day: "2-digit" }).format(d); }
function pubIso(pub) { const d = new Date(pub); return Number.isNaN(d.getTime()) ? null : d.toISOString(); }
...
function notesFromDesc(raw) {
  let t = stripHtml(raw);
  for (const re of [/Hosting provided by/i, /Anchor\.fm/i, /Spotify for Podcasters/i]) {
    const m = t.match(re); if (m && m.index != null) t = t.slice(0, m.index);
  }
  return t.split(/\n+/).map((l) => l.trim()).filter((l) => l && !/^https?:\/\//i.test(l) && !/(贊助|赞助|廣告)/.test(l)).join("\n").trim();
}
function bullets(title, notes) {
  const pts = [];
  if (notes) {
    for (let p of notes.split(/(?<=[。！？!?])\s*|\n+/).map((s) => s.trim()).filter((s) => s.length >= 2)) {
      if (/(贊助|广告|廣告)/.test(p) || /^https?:\/\//i.test(p)) continue;
      if (p.length > 160) p = p.slice(0, 140).replace(/[，,、\s]+$/u, "") + "…";
      if (!pts.includes(p)) pts.push(p);
      if (pts.length >= 6) break;
    }
  }
  if (!pts.length && title) pts.push(String(title).replace(/\s*\|\s*WhynotTV Podcast.*$/i, "").slice(0, 160));
  return pts;
}
function parse(xml) {
  ...
  for (const block of items) {
    ...
    if (ep == null) { const m = title.match(/#(\d+)\s*$/) || title.match(/WhynotTV\s*Podcast\s*#(\d+)/i); ep = m ? +m[1] : null; }
    ...
    episodes.push({
      id: guid || `ep-${ep ?? episodes.length}`, ep, title,
      subtitle: title.replace(/\s*\|\s*WhynotTV Podcast.*$/i, "").trim() || null,
      ...
      keyPoints, notesQuality, source: "anchor-rss",
    });
  }
...
function write(p, data) { fs.mkdirSync(path.dirname(p), { recursive: true }); fs.writeFileSync(p, JSON.stringify(data, null, 2) + "\n"); }
...
const payload = {
  id: "whynottv-episodes", asOf, timezone: "Asia/Taipei", status: "candidate", mathGate: "closed",
  disclaimer: "Not investment advice. Candidate/watch; math gate closed. notesQuality=listened includes industry notes after public audio + STT. RSS-only = show notes only. Host views; no invented tickers/prices/quotes.",
  show: { name: "WhynotTV Podcast", host: "Tairan He", appleId: APPLE_ID, appleUrl: APPLE, siteUrl: SITE, feedUrl: FEED },
  sources: [{ type: "rss", url: FEED, label: "Anchor RSS" }, { type: "apple", url: APPLE, label: "Apple Podcasts" }],
  counts: {
  ...
  listenPipeline: { script: "scripts/listen-whynottv-episodes.mjs", analysisPack: "scripts/whynottv-stock-analysis.json", transcriptModel: "faster-whisper small int8 (zh)", updatedAt: asOf },
  episodes,
};
write(OUT_PUBLIC, payload);
if (fs.existsSync(path.dirname(OUT_DOCS))) write(OUT_DOCS, payload);
console.log(`whynottv episodes=${episodes.length} listened-preserved=${kept} → ${OUT_PUBLIC}`);
